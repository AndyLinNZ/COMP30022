...
describe('Unit Testing: addPlayerToTeam in teamController', () => {
    test('Adding new player with valid id should be added to team.players', async () => {
        const req = mockRequest()
        const res = mockResponse()
        const next = mockNext()
        ...
        req.body = {
            playerIds: ['613472896ef0dc42247c6520'],
        }

        const expectedTeam = new Team({ ...teamDetails, players: ['613472896ef0dc42247c6520'] })

        Player.find = jest.fn().mockResolvedValue([new Player(playerDetails)])
